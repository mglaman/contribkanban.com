...
function BoardListing({ classes }) {
    ...
    function PagerButton({ link, text }) {
        return <Button disableRipple onClick={() => setApiSearchUrl(link.href)} disabled={!link || !link.href}>{text}</Button>
    }
        ...
        return (
            <section>
                ...
                <FormControl fullWidth className={classes.margin}>
                    <TextField label="Search" variant="standard" value={filterName} onChange={event => setFilterName(event.target.value)} />
                </FormControl>
                </form>
                ...
                <List>
                    {boards.data.map(board => {
                        return (
                            <ListItem key={board.id} underline="none" color="inherit" component={RouterLink} to={`/board/${board.attributes.machine_name}`}>
                                <ListItemAvatar>
                                    <Avatar>
                                        <FolderIcon />
                                    </Avatar>
